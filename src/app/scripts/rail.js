...
function buildRail(){
  ...
  SECTIONS.forEach(sec=>{
    ...
    ops.forEach(o=>{
      ...
      // no longer information -- so it only shows when it means something, the
      // same way the structure and game groups below vanish when empty. Add a
      // fourteenth opening with a line that has no deviations and it comes back.
      const tag = lv < 2 ? DX_LEVELS[lv].tag : "";
      html += `<button class="railitem${on?" on":""}" data-go="${o.id}" title="${opLevelTitle(o)}">`
            + `<span class="railtop">${o.name}${tag?`<b class="lvtag lv${lv}">${tag}</b>`:""}</span>`
            + `<span class="eco">${o.eco}</span></button>`;
      opts += `<option value="${o.id}"${on?" selected":""}>${sec.label} — ${o.name}${tag?` · ${tag}`:""}</option>`;
    });
