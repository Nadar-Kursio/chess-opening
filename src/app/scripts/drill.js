...
function dxClearMsg(){
  const d = state.drill;
  d.msg = ""; d.tone = "";
  // Every .dxmsg on the page, not just the drill panel's: read mode renders the
  // same answer outside the panel, and a selector scoped to .drill left it on
  // screen after the mark under it had already gone.
  document.querySelectorAll(".dxmsg").forEach(n=>n.remove());
...
function brTry(from, to){
  ...
  if(next && next.from === from && next.to === to){
    state.sel = null;
    state.drill.msg = ""; state.drill.tone = ""; state.drill.bad = null;
    step(1);
    dxSay(next.san + ".");
    return;
  }
  ...
  const hit = brMatch(state.ply, from, to);
  if(hit >= 0){ state.sel = null; state.drill.bad = null; brEnter(state.ply, hit, "read"); return; }

  const name = mvName(here.fen, from, to);
  const illegal = mvIsLegal(here, from, to) === false;
  if(illegal){
  ...
  // rejected move looks like would teach the colours twice.
  state.drill.bad = {from, to, kind: illegal ? "illegal" : "wrong"};
