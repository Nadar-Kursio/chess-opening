...
- `src/content/explore/*.json` is generated by that skill's `engine-tree.py` and
  committed. Never hand-edit one: the build checks that the tree is a tree, and
  nothing checks that an eval is true. Regenerating is an hour, and the cache
  under `.engine/` makes a re-run pay only for what changed.
