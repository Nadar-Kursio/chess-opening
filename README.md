...
moves, a Stockfish score under the board on every position of the Ruy Lopez,
chess.com-style arrows (including bent knight arrows), and an auto-generated
...
position, counted from real PGN rather than quoted from anywhere. Under the board
is an **eval bar**: what Stockfish makes of the position you are looking at, on the
line, inside a deviation and in a model game alike, with the coach card saying what
the move just played was worth. And you can write
...
src/
  ...
  content/
    ...
    evals/
      ruylopez.json              a Stockfish score per position — generated,
  ...
  app/
    ...
    styles/                      19 stylesheets, concatenated in build.py's order
    ...
    scripts/                     17 scripts, concatenated in build.py's order
...
| `side` | line | `"white"` or `"black"`: the colour the learner plays on this line, overriding the opening's `orientation`. A trap is studied from both chairs, so one opening can drill the attack as White and the defence as Black — the board, the drill and the plan card all follow the line's side, and the variation row wears a chip naming it. |
...
Under every board the course draws is what Stockfish makes of the position on it
— reading a line, stepping through a deviation, replaying a model game. Not a mode
you switch into: a score you have to go and find is one you check once and then
stop checking, and the point of it is the habit. The coach card carries the other
half, what the move just played did to that number:

```
Stockfish   +0.31 after 9.h3, and the move changed nothing.
Stockfish   −1.86 after 9...Bg4 — Black gave up 2.17.
```

The swing is **derived, never stored**: it is the difference between two numbers
already on the page, and a third copy is how they come to disagree. The bands it
colours on are the ones the research skill calibrates authored severities with, so
an engine number and a written `blunder` never contradict each other.

### Where the numbers come from

`src/content/evals/<opening>.json`, generated by the research skill's
`position-evals.py`, committed, and read by the build like every other piece of
content:
...
— the variations, the deep dive, the continuation of every authored deviation, and
the model games. For the Ruy Lopez that is 841 positions out of 1,039 plies, the
difference being transpositions: scores are keyed by **EPD**, like branches and
personal notes, so a position reached twice is searched once.

**One depth for the whole page.** The build refuses two eval files that disagree
on it, because two positions searched to different depths cannot be compared and
comparing them is the entire reason the bar is on screen for both.

**An opening either has scores everywhere or nowhere.** Coverage is gated on the
opening having a file of its own, not on a position happening to be in someone
else's — otherwise a transposition would light the bar up for four plies of an
opening that has none, and a bar that comes and goes inside one variation is worse
than no bar at all. Twelve openings have no file yet and render exactly as they
did before.

**Regenerate after any content edit.** The files are generated *from* the lines,
so moving a move outruns them. The build warns and names the first position it
could not score; `tests/test_content.py` turns that warning into a failure, which
is what stops a variation shipping with a gap in the middle of its bar.
