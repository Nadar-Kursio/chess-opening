...
You can **read** a line, **drill** it — the board asks for each move before it
shows you, with a three-rung hint ladder and a score — or **explore** it, where you
leave the line at any point, play whatever your opponent actually played, and get
...
src/
  ...
  content/
    ...
    explore/
      ruylopez.json              the engine tree Explore mode walks — generated,
...
## Explore mode — when they play something nobody wrote about

`branches` answers the deviations somebody sat down and authored. Explore answers
**every legal move in the position**, because the engine priced all of them at
once, and then lets you keep playing from wherever that lands.

Pick a variation, step to the move they went wrong at, press **Explore**, and put
their move on the board. You get the score, what the move gave away against the
best move there, the same blunder / inaccuracy / playable word the written
deviations use — on the same centipawn bands, so the two never disagree — and the
engine's continuation as buttons you can play out. Under that is the list of
replies to the position you are now in, ranked, each with its own score, so
"what do I do about it" is answered by clicking one.

It plays both colours. Answering a deviation means putting their move on the board
yourself, so nothing is auto-replied and the board takes any legal move from
either side.

### Where the numbers come from

`src/content/explore/<opening>.json`, generated by the research skill's
`engine-tree.py`, committed, and inlined by the build like every other piece of
content:
...
because it reads the result rather than producing it — and because the search
already happened, the eval is on screen the instant the piece lands rather than
after a browser engine has had two seconds to think, and it is a real depth-18
number rather than what a tab could manage in that time.

The trade is that the tree has an **edge**, and the panel says so in its own box
the moment you cross it: past the searched positions there is no score and no
reply list, because a number that quietly stops updating looks exactly like one
that still means something. Two things set where the edge falls:
...
deep dive, and the deep dive simply runs out of tree thirteen moves early. A line
with no tree shows Explore disabled with the reason, rather than hiding a mode
that exists on the variation next to it.

The build does not check that an eval is *true* — nothing could, short of running
the search again. What it does check is that the tree is a tree: spines that
replay legally, boards that are 64 squares, and no move pointing at a position
